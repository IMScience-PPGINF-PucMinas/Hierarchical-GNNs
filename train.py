...
mlflow.set_experiment("BRM - Target 20 nodes")
...
def calculate_metrics (y_pred , y_true , epoch, type) :
    #print ( f " \ n Confusion matrix : \ n { confusion_matrix ( y_pred , y_true ) } " )
...
def main():
    ...
    train_data = Cifar10_graphs(root="data/", hierarchy=False, nodes=20, split='train')#LOAD THE DATASET  
    val_data = Cifar10_graphs(root="data/", hierarchy=False, nodes=20, split='val')#LOAD THE DATASET 
    test_data = Cifar10_graphs(root="data/", hierarchy=False, nodes=20, split='test')
    ...
    model = BRM(node_feature_size=train_data[0].x.shape[1], edge_feature_size=train_data[0].edge_attr.shape[1], num_classes=train_data[0].y.shape[0])
